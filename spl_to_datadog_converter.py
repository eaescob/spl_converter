...
class SPLParser:
    ...
    def parse_query(self, spl_query: str) -> Dict[str, Any]:
        ...
        for cmd, args in pipe_commands:
            if cmd.lower() in self.search_commands:
                components["commands"].append(
                    {"command": cmd.lower(), "arguments": args.strip()}
                )
        ...
        components["event_type"] = self._determine_event_type(
            query, components["security_category"]
        )

        # Analyze correlation patterns
        components["correlation_info"] = self._analyze_correlation_patterns(
            query, components
        )
        ...
        for command in components.get("commands", []):
            ...
            if cmd_name in self.correlation_commands:
                ...
                if cmd_name == "join":
                    ...
                    join_fields = self._extract_join_fields(
                    ...
                    transaction_fields = self._extract_transaction_fields(
                        command.get("arguments", "")
                    )
...
class DatadogAPIValidator:
    ...
    def validate_rule(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        ...
        test_payload = {"ruleQueryPayloads": self._generate_test_payloads(rule)}
...
class SplunkCloudClient:
    """Client for interacting with Splunk Cloud API."""

    def __init__(self, host_url: str, username: str = None, password: str = None):
        self.host_url = host_url.rstrip("/")
        self.username = username or os.getenv("SPLUNK_USERNAME")
        self.password = password or os.getenv("SPLUNK_PASSWORD")

    def get_saved_searches(self) -> List[Dict[str, Any]]:
        """Fetch saved searches from Splunk Cloud."""
        if not self.username or not self.password:
            raise ValueError(
                "Missing Splunk credentials. Set SPLUNK_USERNAME and SPLUNK_PASSWORD environment variables or provide them as parameters."
            )

        url = f"{self.host_url}/services/saved/searches?output_mode=json"
        ...
        try:
            with urllib.request.urlopen(request, timeout=30) as response:
                ...
                # Extract saved searches from response
                saved_searches = []
                for entry in data.get("entry", []):
                    ...
                    # Only include enabled searches that contain actual search logic
                    if (
                        not disabled
                        ...
                        saved_searches.append(
                            {
                                ...
                                "dispatch.earliest_time": content.get(
                                    "dispatch.earliest_time", ""
                                ),
                                "dispatch.latest_time": content.get(
            ...
            try:
                error_data = json.loads(error_body)
                error_msg = error_data.get("messages", [{}])[0].get("text", error_body)
            except (json.JSONDecodeError, IndexError, KeyError):
                error_msg = error_body

            raise Exception(
                f"Failed to fetch saved searches (HTTP {e.code}): {error_msg}"
            )

        except urllib.error.URLError as e:
            raise Exception(f"Network error while connecting to Splunk Cloud: {str(e)}")

        except json.JSONDecodeError as e:
            raise Exception(f"Invalid JSON response from Splunk Cloud: {str(e)}")
...
class DatadogDetectionRuleGenerator:
            ...
            "queries": [
                {
                    ...
                    "aggregation": {
                        "type": "count",
                        "group_by_fields": self._extract_group_by_fields(
                            spl_components
                        ),
                        "metric": "logs",
                    },
    ...
    def _generate_rule_name(self, spl_components: Dict[str, Any]) -> str:
        ...
        if search_terms:
            ...
            relevant_terms = [
                ...
                if len(term) > 3 and term not in ["index", "search", "sourcetype"]
            ]
            if relevant_terms:
    ...
    def _generate_correlated_rule(
        self, spl_components: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        rule = {
            ...
            "filters": self._generate_suppression_filters(spl_components),
            "options": {
                "evaluation_window": self._get_evaluation_window(
                    category, correlation_type
                ),
                "keep_alive": 7200,
    ...
    def _generate_transaction_queries(
        self, spl_components: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_subsearch_queries(
        self, spl_components: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_suppression_filters(
        self, spl_components: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if category == "authentication":
            filters.append({"query": "@user.name:(service-account* OR system OR root)"})
        elif category == "network":
            filters.append(
                {
    ...
    def _get_evaluation_window(
        self, category: str, correlation_type: str = None
    ) -> int:
...
def main():
    ...
    input_group.add_argument("query", nargs="?", help="SPL query to convert")
    input_group.add_argument(
        "--url",
        "-u",
        help="Splunk Cloud host URL to fetch saved searches from (e.g., https://your-instance.splunkcloud.com)",
    )
    ...
    try:
        if args.url:
            # Splunk Cloud mode - fetch saved searches
            print(f"Fetching saved searches from {args.url}...")
            splunk_client = SplunkCloudClient(args.url)
            saved_searches = splunk_client.get_saved_searches()

            if not saved_searches:
                print("No enabled saved searches found.")
                return

            print(
                f"Found {len(saved_searches)} saved searches. Converting to Datadog rules..."
            )
            ...
            for search_info in saved_searches:
                search_name = search_info["name"]
                search_query = search_info["search"]

                print(f"Processing: {search_name}")

                try:
                    # Parse SPL query
                    components = parser_obj.parse_query(search_query)

                    # Generate Datadog detection rule
                    datadog_rule = generator.generate_detection_rule(components)

                    # Add metadata from saved search
                    datadog_rule["name"] = f"{search_name} (Converted from Splunk)"
                    if search_info.get("description"):
                        datadog_rule["message"] += (
                            f"\n\n**Original Description:** {search_info['description']}"
                        )

                    # Add original search as tag
                    datadog_rule["tags"].append("splunk-saved-search")
                    if search_info.get("cron_schedule"):
                        datadog_rule["tags"].append(
                            f"original-schedule:{search_info['cron_schedule']}"
                        )
            ...
            if not all_rules:
                print("No rules were successfully converted.")
                return

            # Validate rules if requested
            if args.validate:
                print("Validating rules with Datadog API...")
                validator = DatadogAPIValidator()
                for rule_info in all_rules:
                    validation_result = validator.validate_rule(
                        rule_info["datadog_rule"]
                    )
                    if validation_result.get("valid"):
                        print(f"✅ {rule_info['original_name']}: Validation successful")
                    else:
                        print(
                            f"❌ {rule_info['original_name']}: {validation_result.get('error')}"
                        )
            ...
            output_data = {
                "conversion_summary": {
                    ...
                    "successfully_converted": len(all_rules),
                    "conversion_timestamp": datetime.now(timezone.utc).isoformat(),
                    "splunk_host": args.url,
                },
                "rules": [rule_info["datadog_rule"] for rule_info in all_rules],
            }
            ...
            if args.output:
                with open(args.output, "w") as f:
                    f.write(json_output)
                print(f"Converted {len(all_rules)} rules written to {args.output}")
            else:
                print(json_output)

        else:
            # Direct SPL query mode
            if not args.query:
                print("Error: Must provide either a query or --url flag")
                return
            ...
            if args.validate:
                ...
                if validation_result.get("valid"):
                    ...
                    if "result" in validation_result:
                        print(
                            f"Validation details: {json.dumps(validation_result['result'], indent=2)}"
                        )
                else:
                    print(
                        f"❌ Rule validation failed: {validation_result.get('error')}"
                    )
                    if (
                        not args.output
                    ):  # Still output the rule even if validation fails
        ...
        if hasattr(args, "validate") and args.validate:
            print(
                "Note: Validation requires DD_API_KEY and DD_APPLICATION_KEY environment variables",
                file=sys.stderr,
            )
        if hasattr(args, "url") and args.url:
            print(
                "Note: Splunk Cloud access requires SPLUNK_USERNAME and SPLUNK_PASSWORD environment variables",
                file=sys.stderr,
            )
        sys.exit(1)
